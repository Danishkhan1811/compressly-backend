...
app.post('/compress', upload.single('image'), async (req, res) => {
  try {
    ...
    await sharp(req.file.path)
      .resize(parseInt(width) || null, parseInt(height) || null) // Resize if provided
    ...
    // Return download link with HTTPS
    res.json({ downloadLink: `https://compressly-backend.onrender.com/download/${path.basename(outputPath)}` });
  } catch (error) {
    console.error('Error compressing image:', error);
    res.status(500).json({ message: 'Error compressing image' });
  }
...
// Route to serve files with a "Content-Disposition" header for download
app.get('/download/:filename', (req, res) => {
  const filename = req.params.filename;
  const filePath = path.join(__dirname, 'optimized', filename);
